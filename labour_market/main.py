...
def process(path_lists):
    ...
    for path in path_lists:
        ...
        for data in datas['features']:
            ...
            code = int(data['properties']['state_code_2016'])
            if code in state_to_name:
                city = state_to_name[code]
                if city not in output_json:
                    output_json[city] = {"details": []}
                group = init_dict.copy()
                for key, value in data['properties'].items():
                    if key in init_dict:
                        try:
                            ...
                            pass
                output_json[city]['details'].append(group)
    ...
    for data in datas['features']:
        ...
        if code in state_to_name:
            city = state_to_name[code]
            summary = {}
            summary['total_pop'] = data['properties']['working_age_pop_15_64']
            summary['employ_rate'] = data['properties']['mpy_rt_15_64']
            summary['unemploy_15'] = data['properties']['unemp_rt_15']
            summary['unemploy_15_24'] = data['properties']['yth_unemp_rt_15_24']
            output_json[city]['summary'] = summary
