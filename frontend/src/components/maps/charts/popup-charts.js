...
export function PopupCharts(props) {
  ...
  useEffect(() => {
    fetch(`http://${endpoint}:3001/tweets/sentiment_score/info?scenario=SY`)
      .then((resp) => resp.json())
      .then((json) => {
        setData(json);
      });
  ...
  function getScore() {
    if (
      data.rows.filter(
        (item) => item.key[0] === STATE_NAME && item.key[1] == year
      ).length === 0
    ) {
      return "";
    }
    ...
    return data.rows
      .filter((item) => item.key[0] === STATE_NAME && item.key[1] == year)[0]
      .value.toFixed(2);
  }
  return (
    <>
